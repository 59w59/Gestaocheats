...
if (!file_exists($upload_dir)) {
    if (!mkdir($upload_dir, 0755, true)) {
        error_log("ERRO: Não foi possível criar o diretório $upload_dir");
        header('Content-Type: application/json');
        echo json_encode(['error' => 'Falha ao criar diretório de uploads', 'success' => false]);
        exit;
    }
    error_log("Diretório de uploads criado: $upload_dir");
}

if (!is_writable($upload_dir)) {
    error_log("ERRO: Diretório $upload_dir não tem permissão de escrita");
    header('Content-Type: application/json');
    echo json_encode(['error' => 'Sem permissão para salvar arquivos no servidor', 'success' => false]);
    exit;
}
...
// Log para debug
error_log("[UPLOAD] Requisição recebida: " . json_encode($_POST) . ", FILES: " . json_encode($_FILES));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
    try {
        ...
        $unique_filename = uniqid('admin_ticket_' . $ticket_id . '_') . '.' . $file_extension;
